...
rsf_data <- genomic_features %>%
  dplyr::select(-PATIENT_ID, -SAMPLE_ID, -CANCER_TYPE)
nrow(rsf_data)
# Fit RSF
set.seed(123)
...
# 1. Fit PAM for k=5
final_pam5 <- pam(dist_obj1, k = 5, diss = TRUE)
# Check cluster sizes
table(final_pam5$clustering)
# 2. Add the k=5 labels to your data
genomic_features$cluster_k5 <- as.factor(final_pam5$clustering)

# 3. Fit survival curve for k=5
fit_k5 <- survfit(Surv(time, event) ~ cluster_k5, data = genomic_features)
...
ggsurvplot(
  ...
  pval = TRUE, 
  risk.table = TRUE,          # Adds a table showing patients at risk
  title = "Survival by Cluster (k=5)",
  legend.labs = paste0("Cluster ", 1:5),
  palette = "jco" ,
  conf.int = TRUE     # Uses a professional color palette
)
...
```{r}
final_pam10 <- pam(dist_obj1, k = 10, diss = TRUE)

# Check cluster sizes
table(final_pam10$clustering)

library(survival)
library(survminer)

# Add cluster labels
genomic_features$cluster_k10 <- as.factor(final_pam10$clustering)

# Fit survival curve
fit_k10 <- survfit(Surv(time, event) ~ cluster_k10, data = genomic_features)

# Plot and check the P-value
ggsurvplot(fit_k10, data = genomic_features, pval = TRUE, 
           title = "Survival by Cluster (k=14)", conf.int = TRUE)

```

chose k=5 because it provided the optimal balance of mathematical stability and clinical utility, yielding distinct, non-overlapping survival trajectories that clearly stratified patients into actionable risk tiers.
...
run_rsf_clustering <- function(data, cancer_type, ntree = 1000, k_range = 2:15) {
  ...
  for (k in k_range) {
    ...
    sil_width_sub[k] <- pam_fit$silinfo$avg.width
  }
  
  optimal_k_sub <- which.max(sil_width_sub)
