...
cluster_vimp_list <- list()

set.seed(123)

for (cl in levels(genomic_features$cluster)) {
  
  # Subset data for cluster
  data_cl <- genomic_features %>%
    filter(cluster == cl)
  
  # Remove IDs + cluster column
  rsf_data_cl <- data_cl %>%
    select(-PATIENT_ID, -SAMPLE_ID, -cluster)
  
  # Fit RSF model within cluster
  rsf_cl <- rfsrc(
    Surv(time, event) ~ .,
    data = rsf_data_cl,
    ntree = 500,
  ...
  # Store VIMP
  cluster_vimp_list[[cl]] <- rsf_cl$importance
}

library(tidyr)

cluster_vimp_df <- bind_rows(
  lapply(names(cluster_vimp_list), function(cl) {
    data.frame(
      variable = names(cluster_vimp_list[[cl]]),
      importance = cluster_vimp_list[[cl]],
      cluster = paste0("Cluster_", cl)
    )
  })
)

cluster_vimp_df <- cluster_vimp_df %>%
  group_by(cluster) %>%
...
ggplot(cluster_vimp_df, aes(x = cluster, y = reorder(variable, norm_importance))) +
  geom_tile(aes(fill = norm_importance)) +
  # Gradient2 creates the diverging scale
  scale_fill_gradient2(
    low = "blue",      # Color for negative VIMP (noise)
    mid = "white",     # Color for zero VIMP
    high = "red",      # Color for positive VIMP (importance)
    midpoint = 0,
    name = "Normalized VIMP"
  ) +
  theme_minimal() +
  labs(
    title = "Cluster-Specific Variable Importance (RSF)",
    subtitle = "Blue indicates negative importance (potential noise)",
    x = "Cluster",
    y = "Features"
  )
```
...
```{r}
top_features <- cluster_vimp_df %>%
  group_by(cluster) %>%
  slice_max(norm_importance, n = 10) %>%
  ungroup()
ggplot(top_features, aes(x = cluster, y = reorder(variable, norm_importance))) +
  geom_tile(aes(fill = norm_importance)) +
  scale_fill_gradient(low = "white", high = "blue") +
  theme_minimal() +
  labs(
    title = "Top 10 Important Features per Cluster",
    x = "Cluster",
    y = "Features"
  )
```
...
get_vimp_df_new <- function(result, cancer_name) {
  data.frame(
    variable = names(result$vimp),
    importance = result$vimp
  ) %>%
...
vimp_df_new <- bind_rows(
  get_vimp_df_new(clustering_results[["Breast Cancer"]], "Breast Cancer"),
  get_vimp_df_new(clustering_results[["Non-Small Cell Lung Cancer"]], "Lung Cancer"),
  get_vimp_df_new(clustering_results[["Colorectal Cancer"]], "Colorectal Cancer")
)
vimp_df_n <- vimp_df_new %>%
  group_by(cancer) %>%
  mutate(norm_importance = importance / max(importance)) %>%
  ungroup()

top_features_new <- vimp_df_n %>%
  group_by(cancer) %>%
  #slice_max(norm_importance, n = 30) %>%
  ungroup()

common_features <- top_features_new %>%
  group_by(variable) %>%
...
ggplot(cox_df, aes(x = estimate, y = reorder(term, estimate))) +
  ...
  labs(
    title = "Multivariable Cox Model: Hazard Ratios",
